...
@st.dialog("📘 특허 질의응답 시스템 매뉴얼")
def show_manual():
    st.markdown("""
    ## 시스템 개요
    본 시스템은 **강남대학교 인공지능융합공학부** 연구팀이 제안한 **'요약-검색 결합 및 다중 문서 참조'** 기술을 기반으로 합니다.  
    기존 키워드 중심 검색의 한계를 극복하고, LLM과 RAG를 결합하여 전문적인 특허 분석 답변을 제공합니다.

    ---

    ### 핵심 기술 (2-Stage Framework)
    우리 시스템은 정확도를 높이기 위해 두 단계로 작동합니다.
    1. **Stage 1 (배경기술 요약 검색)**: 특허의 핵심 기술 정보가 담긴 '배경기술' 요약본을 바탕으로 관련 문서 3건을 빠르게 탐색합니다.
    2. **Stage 2 (전문 기반 답변 생성)**: 검색된 문서의 전문(기술분야, 배경기술, 실시내용)을 **400~500자 단위의 정밀한 청크(Chunk)**로 분석하여 정확한 근거를 추출합니다.

    ---

    ### 시스템의 강점
    * **10% 이상의 성능 향상**: Naive RAG 방식 대비 **LLM-Judge Score 기준 약 10%의 성능 향상**을 입증하였습니다.
    * **정보 부족 및 왜곡 방지**: 단일 문서가 아닌 **다중 문서(Top-3)**를 교차 참조하여 답변의 완전성을 높였습니다.
    * **환각 현상 억제**: GPT-4o-mini 모델을 기반으로 실제 특허 명세서 문헌에 근거한 답변만 생성하도록 설계되었습니다.

    ---

    ### 효과적인 질문 예시
    특허의 구조적 특성을 활용해 질문하면 더 좋은 결과를 얻을 수 있습니다.
    * **기술 비교**: "이 특허와 유사한 선행 기술들의 해결 과제 차이점을 분석해줘."
    * **구조 추출**: "명세서 내 '발명의 실시 내용'을 바탕으로 구체적인 구현 방법을 설명해줘."
    * **핵심 요약**: "배경기술에 언급된 기존 기술의 문제점과 본 발명의 차별점은 뭐야?"

    ---

    ### ⚠️ 안내 및 주의사항
    * **데이터 출처**: 본 시스템은 반도체 분야 등 수집된 특허 데이터셋을 기반으로 합니다.
    * **연구 목적**: 본 앱은 특허 질의응답 품질 향상을 위한 연구용 프로토타입이며, 답변은 법적 효력을 갖지 않습니다.
    """)
